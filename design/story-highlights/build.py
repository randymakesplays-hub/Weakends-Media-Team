...
PACKAGES = [
    dict(slug="01-standard", title="Standard", popular=False,
         best="Your everyday fresh. Start the day in a clean car.",
    ...
    dict(slug="02-gloss", title="Gloss", popular=True,
         best="The one everybody\u2019s actually looking for. A full in and out makeover.",
    ...
    dict(slug="03-deluxe", title="Deluxe", popular=False,
         best="The final boss. Fresh, sealed, basically a brand new car.",
...
def row_html(title, desc, last):
    ...
    return (
        '<div style="padding:31px 0;border-top:2px solid {rule};{edge}">'
...
.best{{margin-top:48px;background:{tint};border-radius:22px;padding:26px 30px;
 min-height:171px;box-sizing:border-box;}}
.best .lbl{{font:500 21px 'IBM Plex Mono',monospace;letter-spacing:0.14em;color:{ink};
 opacity:0.55;}}
.best .txt{{margin-top:10px;font:400 30px/1.34 Archivo,sans-serif;color:{ink};}}
...
<div class="cta"><div class="lbl">CALL OR TEXT</div><div class="num">{phone}</div></div>
        ...
        title=pkg["title"], rows=rows, best=pkg["best"], phone=PHONE)
...
def canvas_section(pkg, idx):
    ...
    for i, (t, d) in enumerate(pkg["rows"]):
        ...
        rows.append(
            '<div style="padding:31px 0;border-top:2px solid %s;%s">'
  ...
  <div style="display:flex;flex-direction:column;gap:22px;flex:0 0 auto;">
    ...
    <section data-screen-label="{label}" style="width:1080px;height:1920px;flex:0 0 auto;position:relative;overflow:hidden;background:#FFFFFF;display:flex;flex-direction:column;">{ribbon}
      ...
      <div style="flex:1 1 auto;padding:44px 84px 0;box-sizing:border-box;display:flex;flex-direction:column;">
        ...
        <div style="margin-top:48px;background:{tint};border-radius:22px;padding:26px 30px;min-height:171px;box-sizing:border-box;">
          <div style="font:500 21px 'IBM Plex Mono',monospace;letter-spacing:0.14em;color:{ink};opacity:0.55;">BEST FOR</div>
          <div style="margin-top:10px;font:400 30px/1.34 'Archivo',sans-serif;color:{ink};text-wrap:pretty;">{best}</div>
        </div>
...
""".format(label=pkg["slug"].replace("-", " ").title(), ink=INK, blue=BLUE,
           ...
           ribbon=ribbon, title=pkg["title"], rows="".join(rows),
           best=pkg["best"], phone=PHONE)
